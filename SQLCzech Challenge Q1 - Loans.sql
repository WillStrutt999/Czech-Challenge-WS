...
select distinct
	...
	,account_id
    ,case
		when datediff(yy,dob,date_loan_granted) between 0 and 9 then '0-9'
		when datediff(yy,dob,date_loan_granted) between 10 and 19 then '10-19'
		when datediff(yy,dob,date_loan_granted) between 20 and 29 then '20-29'
		when datediff(yy,dob,date_loan_granted) between 30 and 39 then '30-39'
		when datediff(yy,dob,date_loan_granted) between 40 and 49 then '40-49'
		when datediff(yy,dob,date_loan_granted) between 50 and 59 then '50-59'
		when datediff(yy,dob,date_loan_granted) between 60 and 69 then '60-69'
		when datediff(yy,dob,date_loan_granted) between 70 and 79 then '70-79'
		when datediff(yy,dob,date_loan_granted) between 80 and 89 then '80-89'
		else '90+'
	end age_bins_loan
	,loan_amount 
from xyz
...
select distinct
	 age_bins_loan
	 ,round(count(*) over (partition by age_bins_loan)/cast(count(*) over() as float)*100,2) percentage_loan_taken
	,avg(loan_amount) over(partition by age_bins_loan) average_loan_debt
from abc
order by average_loan_debt desc